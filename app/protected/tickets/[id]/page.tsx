    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] -mt-8 -mx-8 bg-background overflow-hidden font-sans">
      ...
      <header className="h-16 px-6 border-b bg-background flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 text-sm text-muted-foreground font-medium">
          <Link href="/protected/tickets" className="hover:text-foreground transition-colors font-bold tracking-tight text-[10px]">Mis tickets</Link>
          <ChevronRight className="h-3 w-3 opacity-50" />
          <span className="text-foreground font-black tracking-tight text-[10px]">{displayPortalId}</span>
        </div>
          ...
          <div className="h-full p-6">
            <TicketInfoSidebar 
              ticket={{
                 ...
                 pipeline: hsTicket.properties.hs_pipeline,
